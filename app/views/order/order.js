 ...
 */
 ...
 .controller("orderCtrl",['$scope' , '$rootScope' , '$localStorage', 'toastr' , function($scope, $rootScope, $localStorage, toastr) {
     $scope.orders = [];
     $scope.cafe = {
     ...
     $scope.insert = function () {
         ...
         if( !$scope.cafe.body.value ){
             toastr.info("You must select coffee", "Be careful!");
             return;
         }
         if( !$scope.cafe.sugar.value ){
             toastr.info("You must select sugar baby!", "Watch out...");
             return;
         }
         for( var i in $scope.cafe.extras ){
         ...
         table.insert(obj).done(function(res){
             ...
             toastr.success("Your option has been saved... ^.^", "Inserted");
         });
     }
